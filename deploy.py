...
def server_hashes():
    ...
    names = sorted(set(
        os.path.basename(p) for pat in ("*.py", "*.html", "*.json", "*.txt", "*.sh")
...
def _local_candidates():
    out = []
    for pat in ("*.py", "*.html", "*.json", "*.txt", "*.sh"):
        out += [os.path.basename(p) for p in glob.glob(os.path.join(BASE_DIR, pat))]
    return sorted(set(n for n in out if _is_candidate(n)))
