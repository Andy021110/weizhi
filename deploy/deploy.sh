...
info "配置 cron 定时任务..."
CRON_LINE="${CRON_MINUTE} ${CRON_HOUR} * * * cd ${APP_DIR} && ${APP_DIR}/.venv/bin/python -m weizhi.produce.pipeline >> ${APP_DIR}/run.log 2>&1"
# 若已存在相同条目则跳过，避免重复写入
( crontab -l 2>/dev/null | grep -Fq "weizhi.produce.pipeline" ) || \
  ( crontab -l 2>/dev/null; echo "${CRON_LINE}" ) | crontab -
info "当前 cron 任务："
crontab -l | grep weizhi.produce.pipeline || warn "未检测到 pipeline.py 的 cron 任务，请检查。"
